...
class ApiTokens:
    ...
    def __init__(self, id_api_tokens=None, id_entidad=None, jti=None, tipo_token=None, fecha_expiracion=None,
        ...
        self.revocado = 0 if revocado else 1
    ...
    def insertar(self):
        sql_query = f"INSERT INTO api_tokens (id_usuario, jti, tipo_token, fecha_expiracion, revocado) VALUES " \
                    f"({self.id_entidad}, '{self.jti}', '{self.tipo_token}', '{self.fecha_expiracion}', {self.revocado})"
        print(f'sending query to mySQL: {sql_query}')
        print(sql_query)
        self.cursor.execute(sql_query)
