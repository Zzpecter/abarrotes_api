...
class Usuario():
    ...
    def __init__(self, id_entidad=None, login_usuario=None, password_usuario=None, usuario_registro=None,
    ...
    def insertar(self):
        sql_query = f"INSERT INTO usuario (id_entidad, login_usuario, password_usuario, usuario_registro) VALUES " \
                    f"({self.id_entidad}, '{self.login_usuario}', '{self.password_usuario}', '{self.usuario_registro}')"
        print(f'sending query to mySQL: {sql_query}')
        print(sql_query)
        self.cursor.execute(sql_query)
